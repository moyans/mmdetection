...
def main():
    ...
    line_head = 'name,image_id,confidence,xmin,ymin,xmax,ymax'
    pred_line = []
    pred_line.append(line_head)
    ...
    for idx, filename in enumerate(imgList):
        ...
        stime = time.time()
        predict_dict = Model.run(im, score_thr=0, class_names=class_names)
        etime = time.time()
        ...
        if predict_dict:
            for i, box in enumerate(predict_dict):
                if i == 0:
                    continue
                '''{'bbox': array([1062.6532 ,  833.17676, 1274.5569 , 1028.3533 ], dtype=float32), 'score': 0.9559813, 'name': 'scallop'}'''
                name = box['name']
                image_id = filename.strip().replace('.jpg', '.xml')
                conf = box['score']
                bx = box['bbox'].tolist()
                line = '{},{},{},{},{},{},{}'.format(name, image_id, conf, bx[0], bx[1], bx[2], bx[3])
                
                pred_line.append(line)
                # print(line)
        
    print(pred_line)
    writeLines2Txt(pred_line,'a.csv')
