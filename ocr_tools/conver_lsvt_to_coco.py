...
def convert_lsvt(data_dir, label_file):
    json_file = open(data_dir+label_file,"r")
    lsvt_dataset = json.load(json_file)
    ...
    val_json = os.path.join(data_dir, 'lsvt_val.json')
    ...
    val_idx = []
    for vi, key in enumerate(lsvt_dataset):
        if key in val_key:
            val_idx.append(vi)

    #val_idx = np.random.randint(0,len(lsvt_dataset),size=5000)
    print(val_idx)

    key_id = 0
    for key in lsvt_dataset:
        print(key)
        ...
        img = cv2.imread(data_dir+'train_full_images/'+key+'.jpg')
...
if __name__ == '__main__':
    ...
    if args.labelfile == None:
        args.labelfile = 'train_full_labels.json'
    

    convert_lsvt(args.datadir, args.labelfile)
